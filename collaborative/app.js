(function() {
  ...
  async function uploadRoomImage(dataUrl) {
    if (!roomCode) return dataUrl;
    const res = await fetch('/.netlify/functions/upload', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ code: roomCode, dataUrl })
    });
  ...
  function render() {
    ...
    state.cards.forEach((card) => {
      ...
      head.innerHTML = `
        ...
        <div class="actions">
          ${isCover ? '' : `<button class="icon-btn" data-act="move-up" data-id="${card.id}" title="上移">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="18 15 12 9 6 15"/></svg>
          </button>
          <button class="icon-btn" data-act="move-down" data-id="${card.id}" title="下移">
      ...
      head.querySelectorAll('button[data-act]').forEach(btn => {
        btn.addEventListener('click', (e) => {
          ...
          if (btn.dataset.act === 'move-up') moveCard(card.id, -1);
          if (btn.dataset.act === 'move-down') moveCard(card.id, 1);
        });
      });
  ...
  async function moveCard(id, direction) {
    ...
    if (index < 0 || state.cards[index].type === 'cover') return;
    const target = index + direction;
    if (target < 1 || target >= state.cards.length) {
      showToast(direction < 0 ? '已经是第一张时间卡' : '已经是最后一张时间卡');
      return;
    }

    const [card] = state.cards.splice(index, 1);
    state.cards.splice(target, 0, card);
    editing = { cardId: null, quadIdx: null };
    render();
    await saveToStorage();
  }
